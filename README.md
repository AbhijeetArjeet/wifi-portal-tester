...
A modern, mobile-first Progressive Web App (PWA) built for testing and validating Wi-Fi captive portal authentication systems (`https://captiveportal.kluniversity.in:8090/login.xml`) safely with local CSV testing, **One-Tap Quick Connect**, **Auto-Login on Launch**, and **Failsafe Keep-Alive Auto-Relogin**.
...
- **⭐ Personal Quick-Connect & Local Storage**: Save your personal `Reg.No` and `App.No` locally in your device's browser `localStorage` for instant one-tap logins.
...
2. Enter your `Username (Reg.No)` and `Password (App.No)` under **Personal Quick-Connect & Failsafe**.
